...
```{r, eval=F}
ili1<- read.table('../Data/ili_2020_3_12_nyc.csv',sep="\t", header=TRUE, skipNul = T, fileEncoding="UCS-2LE") # had to add "fileEncoding="UCS-2LE"" 
...
ili1.prop<- read.table('../Data/ili_2020_3_12_nyc_prop.csv',sep="\t", header=TRUE, skipNul = T, fileEncoding="UCS-2LE") # had to add "fileEncoding="UCS-2LE"" 
...
resp1<-read.table('../Data/resp_2020_3_12_nyc.csv',sep="\t", header=TRUE, skipNul = T, fileEncoding="UCS-2LE") 
...
resp1.prop<-read.table('../Data/resp_2020_3_12_nyc_prop.csv',sep="\t", header=TRUE, skipNul = T, fileEncoding="UCS-2LE") 
...
combo1<-combo1[,c('ddate', 'borough', 'agec','ili','resp','denom')]
...
combo2.sub<-combo2[, c('agec', 'ddate','MMWRyear', 'MMWRweek', 'borough', 'ili','resp','flu.rate',"rsv.searches")]
...
glm.func<-function(x.test, age.test, syndrome){
  ...
  ds.glm<-cbind.data.frame('day.of.year'=day.of.year,'y.age'=y.age[1,],'y.age.fit'=y.age.fit,sqrt.rsv, sqrt.flu,  t2,epiyr.index.f, sin1, sin2, cos1, cos2, sin3, cos3)
  ...
  resid1<- log((ds.glm$y.age+0.5) /(pred1+0.5))
...
for(i in 1:5){
  ...
  plot(dates[dates.select],obs.ili[dates.select,j,i], type='l', bty='l', ylab='Fitted', main=paste(j, age.labels[i]), ylim=range(c(ili2.pred.lcl[dates.select,j,i],ili2.pred.ucl[dates.select,j,i],ili2.pred[dates.select,j,i])))
