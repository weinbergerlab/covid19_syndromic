...
date: "3/17/2020 - KC edits:3/13/2020"
...
```{r, eval=F}
ili1<- read.table('../Data/ili_2020_3_16_nyc.csv',sep="\t", header=TRUE, skipNul = T, fileEncoding="UCS-2LE") # had to add "fileEncoding="UCS-2LE"" 
...
ili1.prop<- read.table('../Data/ili_2020_3_16_nyc_prop.csv',sep="\t", header=TRUE, skipNul = T, fileEncoding="UCS-2LE") # had to add "fileEncoding="UCS-2LE"" 
...
resp1<-read.table('../Data/resp_2020_3_16_nyc.csv',sep="\t", header=TRUE, skipNul = T, fileEncoding="UCS-2LE") 
...
resp1.prop<-read.table('../Data/resp_2020_3_16_nyc_prop.csv',sep="\t", header=TRUE, skipNul = T, fileEncoding="UCS-2LE") 
...
```{r}
flu<-read.csv('../Data/FluSurveillance_EIP_New York - Albany_Data 2020_3_16.csv', skip=2)
...
glm.func<-function(x.test, age.test, syndrome){
  ...
  out.list<-list(y=y.age[1,], pred=my_data_with_pi$pred, resid1=resid1, upi=my_data_with_pi$upb, lpi=my_data_with_pi$lpb,'ili.prop'= t(clean.array.citywide[,age.test,'ili.prop']), 'resp.prop'= t(clean.array.citywide[,age.test,'resp.prop'])
 )
  return(out.list)
}
