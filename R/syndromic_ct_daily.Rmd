...
#ds1$pneumonia<- grepl('PNEUMONIA', ds1$classification)
ds1$fever<- grepl('FEVER (INFECTIOUS ', ds1$classification, fixed=T)
...
```{r}
ds1.m<-melt(ds2[,c('adate', 'County','agec','ili','cough','fever','respiratory','circulatory','ili2','all.visits', 'ili2.admit', 'resp.admit','admitted')], id.vars=c('adate','County','agec'))
...
combo2.sub<-combo2[, c('agec', 'adate','MMWRyear', 'MMWRweek', 'County', 'ili2','cough','respiratory','circulatory','fever','flu.rate',"rsv.searches",'all.visits')]
...
glm.func<-function(x.test, age.test, syndrome, all.cause.name){
  ...
  form1<-as.formula(paste0('y.age.fit',"~ epiyr.index.f*sqrt.rsv +   #rsv effect varies by epiyr
                   ...
                   day.of.week+
                   sin1+cos1 +sin2+cos2+ sin3+cos3 +offset(log(offset1[1,]+0.5))"
  ))
...
preds.stage1.regmean<-apply(preds.stage1.regmean,2, function(x) x+offset(log(offset1[1,]+0.5)))
