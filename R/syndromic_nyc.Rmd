...
#```{bash}
#"C:\Program Files (x86)\SaTScan\SaTScanBatch64.exe" "C:\Users\dmw63\Desktop\My documents h\covid19_syndromic\satscan\satscan #parameters\borough temporal.prm"
#```
...
for(i in 1:n.space){
   ...
   for(t in 1:n.time){
      ...
      y_hat[i,t] ~ dnorm(y[i,t], post_var_inv[i,t])
      y[i,t] ~ dnorm(mu[i,t], sigma2_epsilon_inv) 
      mu[i,t] <-         
                 beta0 + beta0_space[i] +  #x[i,t,]%*%gamma + 
...
#Model Organization
##############################################
model_spec<-textConnection(model_string_stage2)
model_jags<-jags.model(model_spec, 
                       inits=list(inits1,inits2, inits3),
                       data=list('y_hat' = t(diff.adult.kids),
                                 'post_var_inv' = 1/t(diff.adult.kids.prec),
                                 'n.space' = ncol(diff.adult.kids),
                                 'n.time'= nrow(diff.adult.kids)),
                       n.adapt=5000, 
                       n.chains=3)

params<-c('theta_space',
          'beta1_space','beta1')
