...
for(i in 1:n.space){
   ...
   for(t in 1:n.time){
      ...
      log(lambda.kid[i,t]) <- alpha1[i] + beta[1,i]*sqrt.flu[t] + beta[2,i]*sqrt.rsv[t] + non.flu.kid[t]
    
      y.adult[i,t] ~ dpois(lambda.adult[i,t])
      log(lambda.adult[i,t]) <- alpha2[i] + delta[1,i]*sqrt.flu[t] + delta[2,i]*sqrt.rsv[t] + non.flu.adult[t]
   ...
   #beta0_space[i] ~ dnorm(0.00, 0.0001)
   #beta1_space[i] ~ dnorm(0.00, 0.0001)
   #theta_space[i] ~ dcat(pi)

}
   
non.flu.kid[1]~  dnorm(0, 1e-4)
non.flu.adult[1]~ dnorm(0, 1e-4)
for(t in 2:n.time){
  non.flu.kid[t]~dnorm(non.flu.kid[t-1], tau.kid)
  non.flu.adult[t]~dnorm(non.flu.adult[t-1], tau.adult)
}
