...
Create index to count number of days from July 1 each year
```{r}
nyc$"o65"<-nyc$`Ages 65+ years`
nyc$"u5"<-nyc$`Ages 0-4 years`
nyc$dow<-as.factor(weekdays(nyc$date))
nyc$epiyr<-year(nyc$date)
nyc$epiyr[month(nyc$date)<=6] <-nyc$epiyr[month(nyc$date)<=6]-1
nyc$jul1.date<-as.Date(paste(epiyr,'07', '01', sep='-'), )
nyc$day.index<- as.numeric(difftime(nyc$date , nyc$jul1.date, unit='days'))
```
