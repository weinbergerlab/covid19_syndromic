...
## Stage 1 model: 
Estimate variation in ILI that is not associated with RSv or flu. Simple poisson model where we have covariates for flu and RSV (with coefficients varying by year)
```{r}
###############
#JAGS Model
###############
...
for(i in 1:n.space){
   ...
   for(t in 1:n.time){
      ...
      log(lambda.kid[i,t]) <- alpha1[i] + epsilon[epiyr.index[i],1]*sqrt.flu[t] + epsilon[epiyr.index[i],2]*sqrt.rsv[t] + t/n.time*epsilon3[i] +
      non.flu.kid[i,t]
    
      y.adult[i,t] ~ dpois(lambda.adult[i,t])
      log(lambda.adult[i,t]) <- alpha2[i] + delta[epiyr.index[i],1]*sqrt.flu[t] + delta[epiyr.index[i],2]*sqrt.rsv[t] + t/n.time*delta3[i]+  non.flu.adult[i,t]
...
for(i in 1:n.space){
 delta3[i]~dnorm(0, tau.delta3)
 epsilon3[i]~dnorm(0, tau.epsilon3)
  non.flu.kid[i,1]   <- 0 #t=0 is captured by intercept
  non.flu.adult[i,1] <- 0
for(t in 2:n.time){
  non.flu.kid[i,t]~dnorm(alpha.kid*non.flu.kid[i,t-1], tau.kid) #note shared tau for all time series
  non.flu.adult[i,t]~dnorm(alpha.adult*non.flu.adult[i,t-1], tau.adult) #note shared tau for all time series
}
}
alpha.kid ~ dunif(0,1)
alpha.adult ~dunif (0,1)
tau.delta3<-1/sd.delta3^2
sd.delta3~dunif(0,100)
tau.epsilon3<-1/sd.epsilon3^2
sd.epsilon3~dunif(0,100)
...
for(k in 1:2){
for(m in 1:n.epiyr){
 epsilon[m,k]~ dnorm(mu.epsilon[k], tau.epsilon[k])
 delta[m,k]~dnorm(mu.delta[k], tau.delta[k])
  } 
 }
...
for(k in 1:2){
 ...
 ## Splines CHECK priors
  for (i in 1:1) { b[i] ~ dnorm(0,0.00049) }
  ## prior for s(weekN)... 
  K1 <- S1[1:13,1:13] * lambda[1] 
  b[2:14] ~ dmnorm(zero[2:14],K1) 
...
```{r}
  ...
  model_jags<-jags.model(model_spec, 
                         ...
                         n.adapt=burnN, 
                         n.chains=3)
  params<-c('diff_hat','non.flu.adult','non.flu.kid', 'lambda.kid', 'lambda.adult')
...
```{r}
matplot(non.flu.adult.mean)
matplot(non.flu.kid.mean)
matplot(diff_hat_mean)
```

## or alternatively use INLA
```{r}
to.test<-c('Bronx','Brooklyn','Manhattan','Queens' ,'Staten Island')
inla.func<-function(x.test){
...
ds.glm<-cbind.data.frame('weekN'=weekN,'y.kids'=y.kids[1,],'y.adult'=y.adult[1,],sqrt.rsv, sqrt.flu, t)
mod1<-inla(y.adult~ sqrt.rsv+sqrt.flu +f(t, model = "rw2") +f(weekN, model = "rw2"), family='poisson', data=ds.glm)
plot(mod1) 
