...
Aggregate by week
```{r}
nyc$weekdate<-floor_date(nyc$date, unit='week')
nyc.agg<-aggregate(nyc[,c('u5','o65')], by=list('weekdate'=nyc$weekdate), FUN=sum)

ratio.wk<-nyc.agg$o65/nyc.agg$u5
```

```{r}
plot(nyc.agg$weekdate, ratio.wk)
```
```{r}
nyc.agg$epiyr<-year(nyc.agg$weekdate)
nyc.agg$epiyr[month(nyc.agg$weekdate)<=6] <-nyc.agg$epiyr[month(nyc.agg$weekdate)<=6]-1
nyc.agg$jul1.date<-as.Date(paste(nyc.agg$epiyr,'07', '01', sep='-'), )
nyc.agg$season.index<- as.numeric(difftime(nyc.agg$weekdate , nyc.agg$jul1.date, unit='weeks'))
nyc.agg$index<-1:nrow(nyc.agg)

mod2<-gam(cbind(nyc.agg$o65, nyc.agg$u5)~ s(season.index, bs='bs') +s(index, bs='bs'), family=binomial, data=nyc.agg)
plot(mod2)
pred2<-predict(mod2, type='response')
raw.ratio<-(nyc.agg$o65+0.5)/(nyc.agg$u5+nyc.agg$o65)
plot(nyc.agg$weekdate, pred2, type='p')
points(nyc.agg$weekdate,raw.ratio, col='red')

obs.exp<-raw.ratio/pred2
plot(nyc.agg$weekdate, obs.exp)
abline(h=1)
```
